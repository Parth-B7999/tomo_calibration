...
%TODO: use ceil(sqrt(sum(WSz.^2)))+1 instead,
...
elseif driftType == 3
    ...
    % (3.1) random Gaussian
    std_ = 0.5; % 0, 0.25, 0.5. 95% within 2*std_, so 95% are within [-0.5, 0.5] or [-1, 1] for std_ = 0.25 or 0.5 
...
%     maxDrift = 1; % driftGT in [-maxDrift, maxDrift]
%     driftGT = imresize(imread('lena512.bmp'), [NTheta, NTau]);  % driftGT = imresize(WGT', [NTheta, NTau]);
    driftGT = (normalize01(driftGT)*2 - 1)*maxDrift;
%     driftGT(driftGT >= 1) = 1 - eps; driftGT(driftGT <= -1 ) = -1 + eps; % threshold large
...
%if ~exist('20190711_brain.mat', 'file'); save('20190711_brain.mat', 'SAll', 'driftGT', 'maxDrift', 'WGT', 'LAll', 'gaussianSTD'); end
if ~exist('20190711_phantom.mat', 'file'); save('20190711_phantom.mat', 'SAll', 'driftGT', 'maxDrift', 'WGT', 'LAll', 'gaussianSTD'); end
...
regWt = 1.3e-3; % 1.3e05 or 1e-5 similar or 1e-8*max(WGT(:))/LNormalizer^2;  1e-6 to 1e-8 both good for phantom, use 1e-8 for brain, especically WGT is scaled to maximum of 1 not 40
regWt = 0.001;
% regWt = 0.0005;
...
paraTwist = {'xSz', WSz, 'regFun', regType, 'regWt', regWt, 'isNonNegative', 1, 'maxIterA', maxIterA, 'xGT', WGT, 'maxSVDofA', maxSVDofA, 'tolA', 1e-4, 'verbose',0};
...
elseif driftType == 3
%     test_old_solver_4_driftType3 = true;
%     if test_old_solver_4_driftType3
%         [WRec, SRec, L, drift, driftAll] = recTompographyWithDrift(S, paraTwist, [], maxDrift, driftGT);
%     else
        % What if we exclude any model error
        % S = interpSinoTypeIII(SAll(:,:,1), driftGT, 'central_diff');  % 'linear' (default), and 'central_diff'     
        
        [WRec_old, SRec_old, drift_old, driftAll_old,info_old] = recTompographyWithDrift_TypeIII(S, paraTwist, WSz, driftGT,10,'old',normeps,0,0);
        [WRec, SRec, drift, driftAll,info] = recTompographyWithDrift_TypeIII(S, paraTwist, WSz, driftGT,10,'new',normeps,1e-4,0);
%    end            
elseif driftType == 4
    % loop of type I and type II
end


figure, 
yyaxis left, plot(info.misfit_history,'linewidth',1.5)
hold on, plot(info_old.misfit_history,'linewidth',1.5)
% xlim([1,10])

yyaxis right, plot(info.psnr_history,'linewidth',1.5)
hold on, plot(info_old.psnr_history,'linewidth',1.5)
% xlim([1,10])
...
figure,
plot(info.lambda_history,'linewidth',1.5)
hold on, plot(info_old.lambda_history,'linewidth',1.5)
legend('new lambdas','old lambdas')
set(gca,'fontsize',16)  
% xlim([1,10])


figure,imagesc(reshape(WUncalib(:,:,3),Ny,Nx)),axis image,axis off
figure,imagesc(reshape(info_old.W_history(:,end),Ny,Nx)),axis image,axis off
figure,imagesc(reshape(info.W_history(:,end),Ny,Nx)),axis image,axis off
